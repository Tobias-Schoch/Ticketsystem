    ...
    <Link to={`/aufgaben/${ticket.id}`} className="block group">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2 sm:mb-3">
          <div className={cn('w-6 sm:w-8 h-1 rounded-full', priorityColors[ticket.priority])} />
          <span className="text-[10px] sm:text-xs text-sage-400 dark:text-sage-500">{formatRelativeTime(ticket.updatedAt)}</span>
        </div>

        {/* Title */}
        <h3 className="font-medium text-sage-800 dark:text-sage-100 line-clamp-2 mb-1.5 sm:mb-2 text-sm sm:text-base transition-colors duration-150 group-hover:text-warmth-600 dark:group-hover:text-warmth-400">
          {ticket.title}
        </h3>

        {/* Description preview */}
        {ticket.description && (
          <p className="text-xs sm:text-sm text-sage-400 dark:text-sage-500 line-clamp-2 mb-3 sm:mb-4">
            {ticket.description}
          </p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {ticket.comments.length > 0 && (
              <div className="flex items-center gap-0.5 sm:gap-1 text-sage-400 dark:text-sage-500 text-[10px] sm:text-xs">
          ...
          <div className="ml-auto">
            {assignee ? (
              ...
              <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-sage-100 dark:bg-sage-700 flex items-center justify-center">
                <User className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-sage-400 dark:text-sage-500" />
              </div>
            )}
          </div>
        </div>
